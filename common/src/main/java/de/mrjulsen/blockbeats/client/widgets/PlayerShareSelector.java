...
public class PlayerShareSelector extends DLGuiComponent {
    ...
    public void refresh(String filter) {
        ...
        ModNetworkManager.GET_USERNAME_CACHE.send(NetworkDirection.toServer(), GetUsernameCachePacket.create((usernamecache) -> {            
            ...
            for (Map.Entry<UUID, PlayerInfo> info : infos.entrySet()) {
                ...
                addComponent(new PlayerWidget(0, dY, width(), info.getValue().getProfile().getId(), info.getValue().getProfile().getName(), info.getValue().getSkin().texture(),
                (b) -> {
    ...
    public void renderMainLayer(DLGuiGraphics graphics, double mouseX, double mouseY, Rectangle renderBounds) {
        DLTextureSheet.DRAGONLIB_UI.getSprite("container").render(graphics, 0, 0, width(), height());
        if (isShared) {
